...
class ShortClasspathStartScriptGenerator {
    ...
    /**
     * The fully qualified application class name.
     */
    ...
    String generateWindowsScriptContent() {
        ...
        def result = generateNativeOutput('windowsStartScript.txt', binding, TextUtil.windowsLineSeparator)
        result = result.replaceFirst(/(set CLASSPATH=%APP_HOME%\\lib\\).*/, { "${it[1]}*" })
        return result
    }
